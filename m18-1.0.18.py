...
#   1.0.18 (2025-10-05): Updated CSV filename format to use dashes for date and time separators (e.g., 2025-10-05-23-14-33) for prettier appearance.
...
class M18:
    ...
    def health(self, force_refresh=True, verbose=False):
        """
        ...
        try:
            ...
            array = self.read_id(reg_list, force_refresh, "array", retries=3)
            ...
            if sn_index is not None and array[sn_index + 1][1]:
                numbers = re.findall(r'\d+\.?\d*', str(array[sn_index + 1][1]))
                if len(numbers) >= 2:
                    bat_type = numbers[0]
                    serial_number = numbers[1]
                    bat_lookup = {
            ...
            if cell_v_index is not None and isinstance(array[cell_v_index + 1][1], list):
            ...
            summary_data.append(["Summary", "E-Serial", serial_number or "------"])
            ...
            cell_voltages = "------"
            if cell_v_index is not None and isinstance(array[cell_v_index + 1][1], list):
                pack_voltage = f"{sum(array[cell_v_index + 1][1])/1000:.2f} V"
                cell_voltages = ", ".join(map(str, array[cell_v_index + 1][1]))
                print(f"Pack Voltage: {pack_voltage}")
                print(f"Cell Voltages (mV): {cell_voltages}")
            ...
            summary_data.append(["Summary", "Pack Voltage", pack_voltage])
            summary_data.append(["Summary", "Cell Voltages (mV)", cell_voltages])
            ...
            temp_non_forge = array[temp_non_forge_index + 1][1] if temp_non_forge_index is not None and array[temp_non_forge_index + 1][1] else "Not available"
            temp_forge = array[temp_forge_index + 1][1] if temp_forge_index is not None and array[temp_forge_index + 1][1] else "Not available"
            ...
            charge_count = "------"
            if redlink_index is not None and dumb_index is not None and total_index is not None:
                charge_count = f"{array[total_index + 1][1]} (Redlink: {array[redlink_index + 1][1]}, Dumb: {array[dumb_index + 1][1]})" if all(isinstance(array[x + 1][1], (int, float)) for x in [redlink_index, dumb_index, total_index]) else "------"
                print(f"Total Charge Count: {charge_count}")
            summary_data.append(["Summary", "Total Charge Count", charge_count])
            time_charge_index = next((i for i, x in enumerate(data_id) if x[0] == 0x9024), None)  # ID 35
            time_charge = array[time_charge_index + 1][1] or "------" if time_charge_index is not None else "------"
            print(f"Total Charge Time: {time_charge}")
            summary_data.append(["Summary", "Total Charge Time", time_charge])
            time_idle_index = next((i for i, x in enumerate(data_id) if x[0] == 0x9028), None)  # ID 36
            time_idle = array[time_idle_index + 1][1] or "------" if time_idle_index is not None else "------"
            ...
            summary_data.append(["Summary", "Time Idling on Charger", time_idle])
            low_v_charge_index = next((i for i, x in enumerate(data_id) if x[0] == 0x902E), None)  # ID 38
            low_v_charge = array[low_v_charge_index + 1][1] or "------" if low_v_charge_index is not None else "------"
            ...
            summary_data.append(["Summary", "Discharge to Empty", str(array[low_voltage_index + 1][1] or "------")])
            ...
            summary_data.append(["Summary", "Overheat Events", str(array[overheat_index + 1][1] or "------")])
            low_voltage_event_index = next((i for i, x in enumerate(data_id) if x[0] == 0x9036), None)  # ID 42
            ...
            summary_data.append(["Summary", "Low Voltage Events", str(array[low_voltage_event_index + 1][1] or "------")])
    ...
    def adv_help(self):
        print("Advanced functions: \n \
            ...
            m.get_snap() - request snapshot (0x61) \n \
